  ...
  try {
    for (const avocat of nouveauxAvocats) {
      ...
      const city = await prisma.city.upsert({
        where: { name: avocat.ville },
        update: {},
        create: {
          name: avocat.ville,
          slug: generateSlug(avocat.ville)
        }
      });
      ...
      const newLawyer = await prisma.lawyer.create({
        data: {
          ...
          langues: ['Français'],
          photo: `http://picturesbytodd.com/wp-content/uploads/2013/07/Head-shots-Lawyer-Professional-Photos-Business-Philadelphia.jpg * 1000) + 1500000000}?w=400&h=400&fit=crop&auto=format`,
          barreau: 'Barreau de ' + avocat.ville,
          consultationDistance: true,
          consultationPhysique: true,
          isActive: true,
          isPremium: false,
          isPartner: false,
